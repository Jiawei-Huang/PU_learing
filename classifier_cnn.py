...
def cons_FC(n_in, n_out, hidden=[15]):
    model = Sequential()

    model.add(Dense(hidden[0], input_dim=n_in, activation='relu'))
...
class CNN(object):
    def __init__(
        ...
        elif self.dataset_name == 'moon':
            self.clf, self.weights = cons_FC(self.img_shape[0], num_category)
